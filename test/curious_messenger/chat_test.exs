defmodule CuriousMessenger.ChatTest do
  use CuriousMessenger.DataCase

  alias CuriousMessenger.Chat

  describe "chat_converstaions" do
    alias CuriousMessenger.Chat.Conversation

    import CuriousMessenger.ChatFixtures

    @invalid_attrs %{" ": nil}

    test "list_chat_converstaions/0 returns all chat_converstaions" do
      conversation = conversation_fixture()
      assert Chat.list_chat_converstaions() == [conversation]
    end
    ...
    test "update_conversation/2 with valid data updates the conversation" do
      ...
      assert conversation.  == "some updated  "
    end
  ...
  describe "chat_converstaions" do
    ...
    test "create_conversation/1 with valid data creates a conversation" do
      valid_attrs = %{" tit=le": "some  tit=le"}

      assert {:ok, %Conversation{} = conversation} = Chat.create_conversation(valid_attrs)
      assert conversation. tit=le == "some  tit=le"
    end
    ...
    test "update_conversation/2 with valid data updates the conversation" do
      conversation = conversation_fixture()
      update_attrs = %{" tit=le": "some updated  tit=le"}

      assert {:ok, %Conversation{} = conversation} = Chat.update_conversation(conversation, update_attrs)
      assert conversation. tit=le == "some updated  tit=le"
    end
